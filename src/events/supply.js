...
async function listenEvent() {
    try {
        ...
        tokenContract.events.Supply(async(error, msg) => {
            if (error) {
                console.error(`[events.supply] SUBSCRIBE ERROR: ${error}`);
                await Sentry.captureException(error);
                throw error
            } else {
                publisherHelper.sendMessage(Conf.Config.RABBIT.QUEUE_SUPPLY, msg)
            }
        });
    } catch (err) {
        console.error(`[events.supply] ERROR: ${err}`);
